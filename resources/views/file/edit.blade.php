          ...
          <div class="col-lg-12">
          	<div class="card">
                ...
                <h3 class="card-title">Edit File</h3>
      				...
      				<div class="card-body">
      				<form action="/file/{{$file->fileid}}/update" method="POST" enctype="multipart/form-data">
      					{{csrf_field()}}
          				<div class="form-group">
          					...
          					<select name="reftypeid" class="form-control" onfocus="this.size=10;" onblur="this.size=1;" onchange="this.size=1; this.blur();">
          						...
          						@foreach($type as $tp)
          							<option value="{{ $tp->typeid }}" {{$file->reftypeid == $tp->typeid ? 'selected' : ''}}>{{ $tp->typename }}</option>
          						@endforeach
          					</select>
          				</div>
                  <div class="form-group">
